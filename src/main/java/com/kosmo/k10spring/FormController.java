...
패키지로 지정된 com.kosmo.k10spring을 스캔하여 컨트롤러 클래스를
찾는다. 그리고 해당 요청명에 매핑되는 메소드를
...
public class FormController {
	...
	/*
	폼값받기2] @RequestMapping 어노테이션으로 폼값받기
	...
	조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼값을 저장할
		객체(DTO 혹은 VO)의 멤버변수의 갯수가 동일할때 사용할수 있다.
	조건2 : 커맨드객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼
		형태의 매개변수를 사용해야 한다.
		Ex) MemberDTO -> memberDTO
	조건3 : 커맨드 객체를 만들때 파라미터의 갯수는 달라도 상관없으나
	...
	파라미터를 마치 경로명처럼 사용하게되고, 이럴때 주의해야할 점은
	웹브라우저는 요청명을 경로로 인식하므로 리소스를 사요할때 경로를
