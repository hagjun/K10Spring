...
public class MemberValidator implements Validator {
	...
	supports()
		...
		못하면 유효성검증일 진행하는 validate()메소드는 아예
	...
	/*
	validate()
		: supports()가 ture를 반환하는 경우에만 호출되는 메소드로
		실제 폼값에 대한 검증을 진행한다.
		매개변수1 : 폼값을 저장한 커맨드객체
	...
	public void validate(Object obj, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, 
				"pw", "pwError");/*
			유효성검증을 위해 전달되는 객체는 커맨드객체가 아니라면
			validate()메소드 내로 진입자체가 불가능하므로, 해당
			함수에서 "pw"라는 문장은 getter()를 호출하여 저장된
			패스워드를 얻어온다고 생각하면 된다.
		*/
	...
	//사용자정의메소드 : 값이 null이거나 길이가 0일때 false반환
